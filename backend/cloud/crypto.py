...
from .models import CloudServiceKey
...
def get_active_service_key():
    key = CloudServiceKey.objects.filter(is_active=True).order_by('-updated_at', '-pk').first()
    if key:
        return key

    generated = generate_rsa_keypair()
    return CloudServiceKey.objects.create(
...
def ensure_service_key(key_id=None):
    if key_id:
        key = CloudServiceKey.objects.filter(key_id=str(key_id), is_active=True).first()
        if key:
            return key
    return get_active_service_key()


def public_key_payload(key):
    if not key:
        return {}
    return {
        'key_id': key.key_id,
